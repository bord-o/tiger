...
STRING_CONTROL | STRING_LONG_ESCAPE | STRING | COMMENT | STRING_ESCAPE | INITIAL
...
fun yyAction42 (strm, lastMatch : yymatch) = (yystrm := strm;
      (YYBEGIN COMMENT;(inc_comment comment_depth); continue()))
fun yyAction43 (strm, lastMatch : yymatch) = (yystrm := strm;
      ((inc_comment comment_depth); continue()))
fun yyAction44 (strm, lastMatch : yymatch) = (yystrm := strm;
      (lineNum := !lineNum+1; linePos := yypos :: !linePos; continue()))
fun yyAction45 (strm, lastMatch : yymatch) = (yystrm := strm; (continue()))
fun yyAction46 (strm, lastMatch : yymatch) = (yystrm := strm;
      ((dec_comment comment_depth);(if !comment_depth = 0 then YYBEGIN INITIAL else ()); continue()))
...
fun yyAction53 (strm, lastMatch : yymatch) = let
      val yytext = yymktext(strm)
      in
        yystrm := strm; (appendString yytext; continue ())
      end
fun yyAction54 (strm, lastMatch : yymatch) = let
      val yytext = yymktext(strm)
      in
        yystrm := strm; (appendString yytext; YYBEGIN STRING; continue ())
      end
...
fun yyAction57 (strm, lastMatch : yymatch) = (yystrm := strm;
      (appendString "\t"; YYBEGIN STRING; continue ()))
fun yyAction58 (strm, lastMatch : yymatch) = (yystrm := strm;
      (YYBEGIN STRING_CONTROL; continue ()))
fun yyAction59 (strm, lastMatch : yymatch) = let
      val yytext = yymktext(strm)
      in
        yystrm := strm;
        (appendString (String.str (chr (valOf (Int.fromString yytext))));
                                YYBEGIN STRING; continue ())
      end
fun yyAction60 (strm, lastMatch : yymatch) = let
      ...
      in
        yystrm := strm;
        (ErrorMsg.error (hd(!linePos)) ("Illegal Escape Expression: " ^ yytext);
                                YYBEGIN STRING; continue ())
      end
...
fun yyAction64 (strm, lastMatch : yymatch) = let
      ...
      in
        yystrm := strm;
        (ErrorMsg.error (hd(!linePos)) ("Improper Multi-line String: " ^ yytext); continue ())
      end
fun yyAction65 (strm, lastMatch : yymatch) = let
      val yytext = yymktext(strm)
      in
        yystrm := strm;
        (appendString (String.str (chr (ord (String.sub(yytext, 0)) - 64)));
                        YYBEGIN STRING; continue ())
      end
        ...
        | SOME(inp, strm') =>
            if inp = #"["
              ...
              then if inp = #":"
                  ...
                  then if inp <= #"/"
                      then yyAction47(strm, yyNO_MATCH)
                      else yyQ70(strm', yyMATCH(strm, yyAction47, yyNO_MATCH))
                else if inp <= #"@"
                  then yyAction47(strm, yyNO_MATCH)
                  else yyQ70(strm', yyMATCH(strm, yyAction47, yyNO_MATCH))
            else if inp = #"`"
       ...
       of NONE => yyAction47(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"_"
              ...
              then if inp = #"`"
                  then yyAction47(strm, yyNO_MATCH)
                  else yyQ112(strm', yyMATCH(strm, yyAction47, yyNO_MATCH))
            else if inp <= #"z"
              then yyQ70(strm', yyMATCH(strm, yyAction47, yyNO_MATCH))
              else yyAction47(strm, yyNO_MATCH)
      (* end case *))
       ...
       of NONE => yyAction1(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"\n"
              ...
              then if inp = #"\t"
                  then yyQ120(strm', yyMATCH(strm, yyAction1, yyNO_MATCH))
...
fun yyQ19 (strm, lastMatch : yymatch) = (case (yygetc(strm))
       of NONE => yyAction45(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"/"
              then yyQ22(strm', yyMATCH(strm, yyAction45, yyNO_MATCH))
              else yyAction45(strm, yyNO_MATCH)
      (* end case *))
...
in
  (case (!(yyss))
   of STRING_CONTROL => yyQ0(!(yystrm), yyNO_MATCH)
    | STRING_LONG_ESCAPE => yyQ1(!(yystrm), yyNO_MATCH)
    | STRING => yyQ2(!(yystrm), yyNO_MATCH)
    | COMMENT => yyQ3(!(yystrm), yyNO_MATCH)
    | STRING_ESCAPE => yyQ4(!(yystrm), yyNO_MATCH)
